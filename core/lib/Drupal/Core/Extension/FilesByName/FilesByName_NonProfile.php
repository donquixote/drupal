...
use Drupal\Core\Extension\SearchdirPrefixes\SearchdirPrefixesBase;
...
class FilesByName_NonProfile extends FilesByNameBase {
  ...
  public function getFilesByName() {
    $files_by_name = parent::getFilesByName();
    if ($this->typeIsModule) {
      // Add active profiles as modules.
      foreach ($this->activeProfileDirs->getProfileDirs() as $profile_name => $profile_dir) {
        $files_by_name[$profile_name] = $profile_dir . '/' . $profile_name . '.info.yml';
      }
    }
  ...
  protected function getSearchdirPrefixWeights() {
    $searchdir_prefix_weights = parent::getSearchdirPrefixWeights();
    $searchdir_prefix_weights['/@_active_profile_extensions_@/'] = SearchdirPrefixesBase::ORIGIN_PROFILE;
