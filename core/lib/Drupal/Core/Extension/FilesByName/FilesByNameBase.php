...
/**
 * Shared base class for most FilesByName* implementations.
 */
abstract class FilesByNameBase implements FilesByNameInterface {
   ...
   */
  function __construct(SearchdirPrefixesInterface $searchdirPrefixes, SearchdirToFilesGroupedInterface $searchdirToFilesGrouped, $extensionType) {
  ...
  protected function getFilesGroupedBySearchdirPrefix() {
    ...
    foreach ($this->searchdirPrefixes->getSearchdirPrefixWeights() as $searchdir_prefix => $searchdir_weight) {
      $files_by_type_and_subdir_and_name = $this->searchdirToFilesGrouped->getFilesGrouped($searchdir_prefix);
